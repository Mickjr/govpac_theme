<?php
/*
Template Name: MOD-Uploads-Edit 
*/
?>
...
<?php 
	...
	if (isset($_GET['upl'])) {
		//Entering this from other page
		$fID = $_GET['upl'];
		$myUpl = 'SELECT * FROM jp_uploads WHERE id = '.$fID;
		// Retrieve Upload and Formatting Dates
		$upData = $wpdb->get_row($myUpl, ARRAY_A);
		
		$upData['message_date'] = date('m/d/Y', strtotime($upData['message_date']));
		
		$filename = basename( get_attached_file( $upData['attachment_id'] ) );
	} 
	...
	if (isset($_POST['submit']))
	{
		...
		// Submitting changes to save into database
							...
							<div class="col-sm-12 col-md-6 col-lg-6">
								<div class="form-group">
									...
									<input 	id="title" name="title" 
											...
											value="<?php if (isset($_POST['title'])) { echo $_GET['title']; } else { echo stripslashes($upData['title']); } ?>" />
								</div>
							</div>						
							<div class="col-sm-12 col-md-6 col-lg-6">
								<div class="form-group">
									...
									<input 	type="text" id="message_date" name="message_date" 
											...
											value="<?php if (isset($_POST['message_date'])) { echo $_GET['message_date']; } else { echo $upData['message_date']; } ?>" />
								</div>
							</div>	
							<div class="col-sm-12 col-md-6 col-lg-6">
								<div class="form-group">
									...
									<input 	id="description" name="description" 
											type="text" class="form-control"
											value="<?php if (isset($_POST['description'])) { echo $_GET['description']; } else { echo stripslashes($upData['description']); } ?>" />
								</div>
							</div>	
							<div class="col-sm-12 col-md-6 col-lg-6">
								<div class="form-group">
									<label for="filepath">File name: </label><?php echo ' ('. $filename .')'; ?>
									...
									<textarea 	type="editor" class="editor form-control" 
										...
										<?php 
											if ($_POST['notes']) {
												...
												if ($upData['notes']) 
												{
													echo stripslashes( $upData['notes'] );
												}
											}
										?>
									</textarea>
